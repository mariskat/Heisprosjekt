...
bool queue_elevator_should_stop(int currentFloor, elev_button_type_t buttons);

//Returns true if there is an order below the current floor
bool queue_orders_downward(currentFloor);
  
//Returns true if there is an order above the current floor
bool queue_orders_upward(currentFloor);
...
//Gets new direction
int elev_motor_direction_t queue_get_new_direction(int last_dirn, int floor);
